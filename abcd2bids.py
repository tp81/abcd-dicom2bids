...
def run_bids_validator(cli_args):
    ...
    subprocess.check_call("date")
    bids_validation_exit_code = subprocess.call([
        "docker", "run", "-ti", "--rm", "-v",
        os.path.abspath(cli_args.output) + ":/data:ro",
        "bids/validator", "/data"
    ])

    # Inform user about whether BIDS validation succeeded
    if bids_validation_exit_code is 0:
        print("\nBIDS validation finished successfully at:")
        subprocess.check_call("date")
    else:
        print("Error: BIDS validation failed.")
        cleanup(cli_args.temp, bids_validation_exit_code)
